  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "GenerateVocabRemapping", name,
        tld.op_callbacks, new_vocab_file, old_vocab_file, "new_vocab_offset",
        new_vocab_offset, "num_new_vocab", num_new_vocab, "old_vocab_size",
        old_vocab_size)
...
def load_and_remap_matrix(ckpt_path, old_tensor_name, row_remapping, col_remapping, initializing_values, num_rows, num_cols, max_rows_in_memory=-1, name=None):
  ...
  * A value of -1 in either of the remappings signifies a "missing" entry. In that
  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "LoadAndRemapMatrix", name,
        tld.op_callbacks, ckpt_path, old_tensor_name, row_remapping,
        col_remapping, initializing_values, "num_rows", num_rows, "num_cols",
