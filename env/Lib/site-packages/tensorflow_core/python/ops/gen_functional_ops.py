...
def fake_param(dtype, shape, name=None):
  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "FakeParam", name,
...
def _for(start, limit, delta, input, body, name=None):
  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "For", name, tld.op_callbacks,
        start, limit, delta, input, "body", body)
      return _result
    except _core._FallbackException:
      try:
...
def _if(cond, input, Tout, then_branch, else_branch, output_shapes=[], name=None):
  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "If", name, tld.op_callbacks,
        cond, input, "Tout", Tout, "then_branch", then_branch, "else_branch",
        else_branch, "output_shapes", output_shapes)
      return _result
    except _core._FallbackException:
      try:
...
def partitioned_call(args, Tout, f, config="", config_proto="", executor_type="", name=None):
  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "PartitionedCall", name,
        tld.op_callbacks, args, "Tout", Tout, "f", f, "config", config,
...
def remote_call(target, args, Tout, f, name=None):
  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "RemoteCall", name,
        tld.op_callbacks, target, args, "Tout", Tout, "f", f)
      return _result
    except _core._FallbackException:
      try:
...
def stateless_while(input, cond, body, output_shapes=[], parallel_iterations=10, name=None):
  ...
  if tld.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, tld.device_name, "StatelessWhile", name,
